...
cmd+="-jar metacherchant.jar -t triple-reads-classifier "
if [[ $k ]]; then
    cmd+="-k $k "
fi
if [[ $k2 ]]; then
...
$cmd6
if [[ $? -eq 0 ]]; then
    cat "$w/6/reads_classifier/found_1.fastq" >> "$o/strain_from_before_1.fastq"
    cat "$w/6/reads_classifier/found_2.fastq" >> "$o/strain_from_before_2.fastq"
    cat "$w/6/reads_classifier/found_s.fastq" >> "$o/strain_from_before_s.fastq"
